...
def get_individual_segments_librosa(data_folder, filename, input_sec=8, sample_rate=16000, hop_sec=2,
                                    butterworth_filter=None, spectrogram=False):
    sample_data = []
...
def add_noise_to_audio(data, rate, noise_type, noise_db):
    if noise_type == 'gaussian':
        ...
        for babble_source_file in babble_sources_files:
            babble_sources.append(librosa.load(babble_source_file, sr=rate)[0])
...
def get_entire_signal_librosa(data_folder, filename, input_sec=8, sample_rate=16000, butterworth_filter=None,
                              spectrogram=False, pad=False, from_cycle=False, yt=None, types='repeat',
                              noise_type=None, noise_db=None):
    if not from_cycle:

        # load file with specified sample rate (also converts to mono)
        data, rate = librosa.load(os.path.join(data_folder, filename), sr=sample_rate)

        if noise_type:
            data = add_noise_to_audio(data, rate, noise_type, noise_db)
...
def crop_window_around_peak(audio, sr, window_duration=8.18):
...
def get_entire_signal_librosa_trimmed(data_folder, filename, input_sec=8.18, sample_rate=16000, butterworth_filter=None,
                              spectrogram=False, pad=False, from_cycle=False, yt=None, types='repeat'):

    # load file with specified sample rate (also converts to mono)
    data, rate = librosa.load(os.path.join(data_folder, filename), sr=sample_rate)

    yt = crop_window_around_peak(data, rate, window_duration=input_sec)
    ...
    if duration < input_sec:
        if not pad:
            print("Warning: audio too short, skipped. Duration: ", duration)
            return None
        else:
            yt = split_pad_sample([yt, 0, 0], input_sec, sample_rate, types)[0][0]
    # else:
    #     required_samples = int(input_sec * sample_rate)
    #     yt = yt[:required_samples]
...
def get_split_signal_librosa(data_folder, filename, input_sec=8, sample_rate=16000, butterworth_filter=None,
                             spectrogram=False, trim_tail=False):
    # print(os.path.join(data_folder, filename+'.wav'))

    # load file with specified sample rate (also converts to mono)
    data, rate = librosa.load(os.path.join(
        data_folder, filename), sr=sample_rate)
...
def get_individual_cycles_librosa(class_split, recording_annotations, data_folder, filename, sample_rate, n_cls,
...
def get_split_signal_fbank_pad(data_folder, filename, input_sec=8, sample_rate=16000, butterworth_filter=None,
                               spectrogram=False, trim_tail=False):
    ...
    for waveform in audio_chunks:
        ...
        if waveform.shape[1] > 400:
            fbank = torchaudio.compliance.kaldi.fbank(waveform, channel=0, frame_length=25, htk_compat=True,
                                                      sample_frequency=sample_rate, use_energy=False,
